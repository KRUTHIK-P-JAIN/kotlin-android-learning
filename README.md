# kotlin-android-learning

My learning journey documenting **Kotlin**, **Android**, **Clean Architecture**, **CI/CD**, **Testing**, **KMM**, and **AI integrations**.

---

## 📂 Repository Structure
```
kotlin-android-learning/
│
├── README.md                   # Root README (this file)
└── app/
    ├── src/main/java/com/example/kotlin_android_learning/
    │   └── phase1_kotlin/       # Phase 1: Kotlin foundations
    │
    └── build.gradle
```

---

## 🚀 Learning Phases

1. **Phase 1 - Kotlin Fundamentals**
    - Core Kotlin concepts with mini tasks and examples.
    - Detailed topics and progress tracked in [phase1_kotlin/README.md](app/src/main/java/com/example/kotlin_android_learning/phase1_kotlin/README.md).

2. **Phase 2 - Android & Clean Architecture**
    - Build scalable Android apps following modern practices.
    - Includes modularization, Compose, and architecture patterns.

3. **Phase 3 - Advanced Topics**
    - CI/CD pipelines, Testing frameworks, KMM basics, AI API integrations.

---

## 📅 Next Steps
- Complete **Phase 1 Kotlin topics**
- Move to **Phase 2 Android & Clean Architecture**
- Progressively add **CI/CD, Testing, KMM, AI integration**

---

## 🎯 Goal
- Build **hands-on experience** with each topic
- Keep a **clear documented learning path**
- Prepare for **interviews and real-world app development**
